...
print_usage() {
  cat <<'EOF'
Usage:
  bash dash_scripts/run_experiments.sh [options]
...
Options:
  ...
  -f, --fast                 Force fast cache simulator path (default)
      ...
      --results-root DIR     Override experiment output root (default: dash_results/experiments)
...
RESULTS_ROOT="${RESULTS_ROOT:-dash_results/experiments}"
...
dataset_log_path() {
  local dataset="$1"
  echo "dash_dataset/synthetic/${PROTOCOL}/log_${dataset}.txt"
}

dataset_csv_path() {
  local dataset="$1"
  echo "dash_results/synthetic/${PROTOCOL}/cache_sim/route_changes/results_synthetic_${dataset}.csv"
}
...
ensure_dataset_log() {
  ...
  if [[ "$NO_GENERATE" == "1" ]]; then
    fail_no_generate "dataset log ${log_path}" "bash dash_scripts/run_htsim_synthetic.sh --protocol ${PROTOCOL} --dataset ${dataset}"
  fi
  ...
  if is_flow_size_dataset "$dataset"; then
    ...
    GENERATE_FLOW_SIZE_SWEEP=1 FLOW_SIZE_SWEEP_VALUES="$fs" \
      bash dash_scripts/run_htsim_synthetic.sh --protocol "$PROTOCOL" --dataset "$dataset"
    return
  fi
...
ensure_route_csv() {
  ...
  if [[ "$NO_GENERATE" == "1" ]]; then
    fail_no_generate "route-change CSV ${csv_path}" "bash dash_scripts/run_cache_sim_synthetic.sh --protocol ${PROTOCOL} --dataset ${dataset}"
  fi

  echo "  generating missing cache CSV: ${dataset}"
  if [[ "$FAST" == "1" ]]; then
    bash dash_scripts/run_cache_sim_synthetic.sh --protocol "$PROTOCOL" --dataset "$dataset" --fast
  else
    bash dash_scripts/run_cache_sim_synthetic.sh --protocol "$PROTOCOL" --dataset "$dataset" --no-fast
  fi
}
...
ensure_flow_stats() {
  ...
  python3 dash_scripts/flow_stats.py "$logfile" --top 20 --concurrency --csv "$out_csv" > "$out_txt"
}
...
plot_csv_if_missing() {
  ...
  python3 dash_scripts/plot_cache.py "${args[@]}"
}
...
for ds in datasets:
    txt = f"dash_results/synthetic/{protocol}/flow_stats/{ds}_flow_stats.txt"
...
generate_comparison_plots() {
  ...
  python3 dash_scripts/plot_experiment_comparisons.py "${args[@]}"
}
