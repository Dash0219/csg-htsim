...
def sweep_splitwise(
    files: Sequence[Path], source_key: str, route_key: str, max_records: int, use_fast: bool = False
    ...
    for idx, path in enumerate(files, start=1):
        ...
        split_results = sweep(records, use_fast=use_fast)
...
def main() -> None:
    ...
    if args.sweep:
        split_outputs: List[Tuple[Path, List[CacheResult]]] = []
        if splitwise:
            results, split_outputs = sweep_splitwise(files, args.source_key, args.route_key, args.max_records, use_fast=args.fast)
        else:
