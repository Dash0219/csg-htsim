...
print_usage() {
  cat <<'EOF'
Usage:
  bash dash_scripts/run_plot_cache_imc.sh [OUT_BASE] [options]
...
stats_file_for_stem() {
  local ds="$1"
  local stem="$2"
  local stats_dir="dash_results/imc/$ds/flow_stats"
  if [[ ! -d "$stats_dir" ]]; then
    return
  fi
...
for ds in univ1 univ2; do
  ...
  split_dir="dash_results/imc/$ds/cache_sim/source_seen/results_${ds}_source_seen_top8_splits"
  if [[ -d "$split_dir" ]]; then
  ...
  total_csv="dash_results/imc/$ds/cache_sim/source_seen/results_${ds}_source_seen_top8.csv"
  if [[ -f "$total_csv" ]]; then
