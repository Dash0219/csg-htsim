...
Accepted dataset names/patterns for --dataset:
  Base datasets:
    ...
    incast_pareto | a2a_pareto | incast_exponential_skewed | a2a_exponential_skewed
  Sweep datasets:
    (incast|a2a)_mono_<N>
    (incast|a2a)_mono_burst_<N>
    (incast|a2a)_pareto_alpha_<A>    (A tokenized, e.g. 1p0, 1p5)
    (incast|a2a)_pareto_temp_<N>
  Sweep shorthand:
    ...
    a2a_pareto_temp_n / incast_pareto_temp_n   (all pareto_temp_<N> sweep datasets for one topology)
...
Environment variables:
  ...
  BURST_MONO_PACKETS    Fixed mono packet count used by burst sweep datasets (default: 256)
...
BURST_MONO_PACKETS="${BURST_MONO_PACKETS:-256}"
...
# Stdout progress/debug output is written to dash_results/synthetic/<protocol>/raw_logs/*.raw.log.
# Output files are written to:
#   dash_dataset/synthetic/<protocol>/log_incast_mono.txt
#   dash_dataset/synthetic/<protocol>/log_a2a_mono.txt
#   dash_dataset/synthetic/<protocol>/log_incast_bimodal.txt
#   dash_dataset/synthetic/<protocol>/log_a2a_bimodal.txt
#   dash_dataset/synthetic/<protocol>/log_incast_pareto.txt
#   dash_dataset/synthetic/<protocol>/log_a2a_pareto.txt
#   dash_dataset/synthetic/<protocol>/log_incast_exponential_skewed.txt
#   dash_dataset/synthetic/<protocol>/log_a2a_exponential_skewed.txt
# Optional temporal-locality variants:
#   dash_dataset/synthetic/<protocol>/log_<pattern>_temp_<multiplier>.txt
...
SIM_BIN="./htsim_${PROTOCOL}"
if [[ ! -x "$SIM_BIN" ]]; then
  echo "ERROR: $SIM_BIN not found or not executable. Build first with: make all"
  exit 1
fi

OUT_DIR="dash_dataset/synthetic/${PROTOCOL}"
CM_DIR="dash_results/synthetic/${PROTOCOL}/connection_matrices"
RUN_DIR="dash_results/synthetic/${PROTOCOL}/raw_logs"
...
if [[ -n "$DATASET" && "$DATASET" =~ ^(incast|a2a)_mono_n$ ]]; then
  ...
  GENERATE_TEMPORAL_SWEEP=0
elif [[ -n "$DATASET" && "$DATASET" =~ ^(incast|a2a)_pareto_temp_n$ ]]; then
  DATASET_FAMILY_PREFIX="${BASH_REMATCH[1]}_pareto_temp_"
  DATASET_FAMILY_KIND="pareto_temp"
  DATASET=""
  GENERATE_FLOW_SIZE_SWEEP=0
  GENERATE_BURST_SWEEP=0
  GENERATE_PARETO_SKEW_SWEEP=0
...
is_valid_dataset() {
  local name="$1"
  if [[ "$name" =~ ^(incast|a2a)_(mono|mono_burst|pareto_alpha|pareto_temp)_n$ ]]; then
    return 0
  fi
  ...
  case "$name" in
    incast_mono|a2a_mono|incast_bimodal|a2a_bimodal|incast_pareto|a2a_pareto|incast_exponential_skewed|a2a_exponential_skewed)
      return 0
      ;;
    *)
      return 1
  ...
  if [[ -n "$DATASET_FAMILY_PREFIX" ]]; then
    case "$DATASET_FAMILY_KIND" in
      ...
      pareto_alpha)
        ...
        ;;
      pareto_temp)
        [[ "$name" =~ ^(incast|a2a)_pareto_temp_[0-9]+$ ]] && [[ "$name" == "$DATASET_FAMILY_PREFIX"* ]]
        return
        ;;
    esac
  fi
...
if [[ "$GENERATE_PARETO_SKEW_SWEEP" == "1" ]]; then
  ...
  for alpha in "${PARETO_ALPHA_SWEEP_LIST[@]:-}"; do
    ...
    if should_process "incast_pareto_alpha_${alpha_tok}"; then
      run_gen python3 connection_matrices/gen_incast.py "$inc_cm" "$NODES" "$CONNS_INCAST" "$FLOWSIZE_INCAST" "$EXTRA_START_US" "$SEED" "$spec"
      normalize_cm_total_bytes "$inc_cm" "$inc_target"
    fi
    if should_process "a2a_pareto_alpha_${alpha_tok}"; then
      if (( A2A_PARALLEL == 1 )); then
        run_gen python3 connection_matrices/gen_serial_alltoall.py "$a2a_cm" "$NODES" "$A2A_CONNS" "$A2A_GROUPSIZE" "$FLOWSIZE_A2A" "$EXTRA_START_US_A2A" "$SEED" "$spec"
      else
        run_gen python3 connection_matrices/gen_serialn_alltoall.py "$a2a_cm" "$NODES" "$A2A_CONNS" "$A2A_GROUPSIZE" "$A2A_PARALLEL" "$FLOWSIZE_A2A" "$EXTRA_START_US_A2A" "$SEED" "$spec"
      fi
...
if [[ "$GENERATE_FLOW_SIZE_SWEEP" == "1" ]]; then
  ...
  for fs in "${FLOW_SIZE_SWEEP_LIST[@]}"; do
    ...
    if should_process "incast_mono_${fs}"; then
      run_gen python3 connection_matrices/gen_incast.py "$inc_cm" "$NODES" "$FLOW_SWEEP_CONNS_INCAST" "$flow_bytes" "$FLOW_SWEEP_EXTRA_START_US" "$SEED" fixed
    fi

    if should_process "a2a_mono_${fs}"; then
      if (( FLOW_SWEEP_A2A_PARALLEL == 1 )); then
        run_gen python3 connection_matrices/gen_serial_alltoall.py "$a2a_cm" "$NODES" "$FLOW_SWEEP_A2A_CONNS" "$FLOW_SWEEP_A2A_GROUPSIZE" "$flow_bytes" "$FLOW_SWEEP_EXTRA_START_US_A2A" "$SEED"
      else
        run_gen python3 connection_matrices/gen_serialn_alltoall.py "$a2a_cm" "$NODES" "$FLOW_SWEEP_A2A_CONNS" "$FLOW_SWEEP_A2A_GROUPSIZE" "$FLOW_SWEEP_A2A_PARALLEL" "$flow_bytes" "$FLOW_SWEEP_EXTRA_START_US_A2A" "$SEED"
      fi
    fi
  done
fi
...
if [[ "$GENERATE_FLOW_SIZE_SWEEP" == "1" ]]; then
  ...
  for fs in "${FLOW_SIZE_SWEEP_LIST[@]}"; do
    sweep_end_us_incast="$FLOW_SWEEP_END_US_INCAST"
    sweep_end_us_a2a="$FLOW_SWEEP_END_US_A2A"
    if [[ -z "$sweep_end_us_incast" ]]; then
      sweep_end_us_incast=$((FLOW_SWEEP_END_US_INCAST_BASE * fs))
    fi
    if [[ -z "$sweep_end_us_a2a" ]]; then
      sweep_end_us_a2a=$((FLOW_SWEEP_END_US_A2A_BASE * fs))
    fi
    run_htsim "incast_mono_${fs}" "$CM_DIR/incast_mono_${fs}.cm" "$FLOW_SWEEP_CONNS_INCAST" "$sweep_end_us_incast"
    run_htsim "a2a_mono_${fs}" "$CM_DIR/a2a_mono_${fs}.cm" "$FLOW_SWEEP_A2A_CONNS" "$sweep_end_us_a2a"
  done
fi
  ...
  for bs in "${BURST_SWEEP_LIST[@]}"; do
    if [[ "$PROTOCOL" == "ndp" ]]; then
      ...
      if should_process "incast_mono_burst_${bs}"; then
        run_gen python3 connection_matrices/gen_incast.py "$inc_cm" "$NODES" "$CONNS_INCAST" "$burst_flow_bytes" "$EXTRA_START_US" "$SEED" fixed
      fi
      if should_process "a2a_mono_burst_${bs}"; then
        if (( A2A_PARALLEL == 1 )); then
          run_gen python3 connection_matrices/gen_serial_alltoall.py "$a2a_cm" "$NODES" "$A2A_CONNS" "$A2A_GROUPSIZE" "$burst_flow_bytes" "$EXTRA_START_US_A2A" "$SEED"
        else
          run_gen python3 connection_matrices/gen_serialn_alltoall.py "$a2a_cm" "$NODES" "$A2A_CONNS" "$A2A_GROUPSIZE" "$A2A_PARALLEL" "$burst_flow_bytes" "$EXTRA_START_US_A2A" "$SEED"
        fi
      fi
    ...
    if should_process "incast_mono_burst_${bs}"; then
      run_gen python3 connection_matrices/gen_incast.py "$inc_cm" "$NODES" "$CONNS_INCAST" "$burst_flow_bytes" "$inc_extra" "$SEED" fixed
    fi
    if should_process "a2a_mono_burst_${bs}"; then
      if (( A2A_PARALLEL == 1 )); then
        run_gen python3 connection_matrices/gen_serial_alltoall.py "$a2a_cm" "$NODES" "$A2A_CONNS" "$A2A_GROUPSIZE" "$burst_flow_bytes" "$a2a_extra" "$SEED"
      else
        run_gen python3 connection_matrices/gen_serialn_alltoall.py "$a2a_cm" "$NODES" "$A2A_CONNS" "$A2A_GROUPSIZE" "$A2A_PARALLEL" "$burst_flow_bytes" "$a2a_extra" "$SEED"
      fi
    fi
...
if [[ "$GENERATE_TEMPORAL_SWEEP" == "1" ]]; then
  echo
  echo "== Generating temporal-locality sweep datasets =="
  for tv in "${TEMPORAL_SWEEP_LIST[@]}"; do
    inc_cm="$CM_DIR/incast_pareto_temp_${tv}.cm"
    a2a_cm="$CM_DIR/a2a_pareto_temp_${tv}.cm"
    ...
    if should_process "incast_pareto_temp_${tv}"; then
      run_gen python3 connection_matrices/gen_incast.py "$inc_cm" "$NODES" "$CONNS_INCAST" "$FLOWSIZE_INCAST" "$inc_extra" "$SEED" "$PARETO_SPEC"
    fi
    if should_process "a2a_pareto_temp_${tv}"; then
      if (( A2A_PARALLEL == 1 )); then
        run_gen python3 connection_matrices/gen_serial_alltoall.py "$a2a_cm" "$NODES" "$A2A_CONNS" "$A2A_GROUPSIZE" "$FLOWSIZE_A2A" "$a2a_extra" "$SEED" "$PARETO_SPEC"
      else
        run_gen python3 connection_matrices/gen_serialn_alltoall.py "$a2a_cm" "$NODES" "$A2A_CONNS" "$A2A_GROUPSIZE" "$A2A_PARALLEL" "$FLOWSIZE_A2A" "$a2a_extra" "$SEED" "$PARETO_SPEC"
      fi
    fi

    run_htsim "incast_pareto_temp_${tv}" "$inc_cm" "$CONNS_INCAST"
    run_htsim "a2a_pareto_temp_${tv}" "$a2a_cm" "$A2A_CONNS" "$temporal_end_us_a2a"
  done
fi
