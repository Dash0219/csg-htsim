...
Options:
  ...
  -m, --mode MODE           route_changes | source_seen | all (default: all)
...
if [[ "$MODE" != "route_changes" && "$MODE" != "source_seen" && "$MODE" != "all" ]]; then
  echo "ERROR: --mode must be route_changes, source_seen, or all" >&2
  exit 1
fi
...
OUT_DIR="${OUT_DIR:-dash_results/synthetic/cache_sim}"
mkdir -p "$OUT_DIR"
...
infer_dataset_max_concurrency() {
  ...
  local stats_file="dash_results/synthetic/flow_stats/${dataset_name}_flow_stats.txt"
...
if [[ "$MODE" == "route_changes" || "$MODE" == "all" ]]; then
  plot_mode "route_changes" \
    "dash_results/synthetic/cache_sim/route_changes" \
...
if [[ "$MODE" == "source_seen" || "$MODE" == "all" ]]; then
  plot_mode "source_seen" \
    "dash_results/synthetic/cache_sim/source_seen" \
    "" \
