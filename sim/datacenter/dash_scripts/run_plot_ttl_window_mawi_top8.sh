...
print_usage() {
  cat <<'EOF'
Usage:
  bash dash_scripts/run_plot_ttl_window_mawi_top8.sh [options]

Options:
  -s, --split-dir DIR        Input split directory (default: dash_dataset/mawi/mawi_sinks_top8_prefix16)
  -o, --out-dir DIR          Output directory (default: dash_results/mawi/ttl_window/plots)
...
OUT_DIR="${OUT_DIR:-dash_results/mawi/ttl_window/plots}"
...
if [[ "$PER_SINK" == "1" ]]; then
  for sink_file in "${files[@]}"; do
    python3 dash_scripts/plot_ttl_windows.py "$sink_file" \
      --out-dir "$OUT_DIR" \
      --prefix "$PREFIX" \
  ...
  python3 dash_scripts/plot_ttl_windows.py "${files[@]}" \
    --merge-inputs \
