...
print_usage() {
  cat <<'EOF'
Usage:
  bash dash_scripts/run_flow_stats_imc.sh [options]
...
for ds in univ1 univ2; do
  ...
  if [[ ${#logs[@]} -gt 0 ]]; then
    python3 dash_scripts/flow_stats.py "${logs[@]}" --top "$TOP_N" --concurrency --csv "$out_dir/${ds}_all_flow_stats.csv" > "$out_dir/${ds}_all_flow_stats.txt"
  fi
done
