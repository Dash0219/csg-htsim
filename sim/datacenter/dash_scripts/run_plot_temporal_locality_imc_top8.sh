...
print_usage() {
  cat <<'EOF'
Usage:
  bash dash_scripts/run_plot_temporal_locality_imc_top8.sh [options]
...
for ds in univ1 univ2; do
  ...
  for sink_file in "$split_dir"/*.txt "$split_dir"/*.log; do
    ...
    plot_cmd=(python3 dash_scripts/plot_temporal_locality.py "$sink_file" --out-dir "$out_dir" --prefix "${PREFIX}_${ds}" --max-records "$MAX_RECORDS" --max-plot-intervals "$MAX_PLOT_INTERVALS")
