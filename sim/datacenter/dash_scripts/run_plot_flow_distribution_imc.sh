...
Options:
  ...
  -o, --out-base DIR         Output base directory (default: dash_results/imc)
      --prefix NAME          Output filename prefix base (default: sink_flow_distribution)
...
PREFIX="${PREFIX:-sink_flow_distribution}"
COLS="${COLS:-4}"
...
for ds in univ1 univ2; do
  ...
  out_dir="$OUT_BASE/$ds/flow_distribution/plots"
  mkdir -p "$out_dir"

  echo "== plotting IMC flow distribution for $ds =="
...
echo "Done. Plotted flow distributions for $count IMC dataset(s) into $OUT_BASE/{univ1,univ2}/flow_distribution/plots"
