...
Options:
  ...
  -o, --out-dir DIR         Output directory (default: dash_results/synthetic/<protocol>/flow_stats)
...
is_supported_dataset_name() {
  ...
  if [[ "$core_name" =~ ^(incast|a2a)_(mono|bimodal|pareto|exponential_skewed)$ ]]; then
    return 0
  fi
...
OUT_DIR="${OUT_DIR:-dash_results/synthetic/${PROTOCOL}/flow_stats}"
