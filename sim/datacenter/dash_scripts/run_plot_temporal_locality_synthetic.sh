...
print_usage() {
  cat <<'EOF'
Usage:
  bash dash_scripts/run_plot_temporal_locality_synthetic.sh [options]
...
Options:
  ...
  -o, --out-dir DIR         Output directory (default: dash_results/synthetic/temporal_locality/plots)
...
OUT_DIR="${OUT_DIR:-dash_results/synthetic/temporal_locality/plots}"
...
cmd=(python3 dash_scripts/plot_temporal_locality.py --out-dir "$OUT_DIR" --prefix "$PREFIX" --max-records "$MAX_RECORDS" --max-plot-intervals "$MAX_PLOT_INTERVALS")
if [[ -n "$DATASET" ]]; then
  cmd+=(--dataset "$DATASET")
fi
