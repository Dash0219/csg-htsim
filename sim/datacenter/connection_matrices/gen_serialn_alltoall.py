...
if len(sys.argv) not in (9, 10):
    print("Usage: python gen_serialn_alltoall.py <filename> <nodes> <conns_per_group> <groupsize> <parallel_cons> <flowsize> <extrastarttime> <randseed> [size_dist]")
    sys.exit()
filename = sys.argv[1]
...
size_dist_spec = sys.argv[9] if len(sys.argv) == 10 else "fixed"
