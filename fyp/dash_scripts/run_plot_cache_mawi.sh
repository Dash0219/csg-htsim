...
print_usage() {
  cat <<'EOF'
Usage:
  bash fyp/dash_scripts/run_plot_cache_mawi.sh [OUT_BASE] [options]
...
OUT_BASE="${OUT_BASE:-fyp/dash_results/mawi}"
...
stats_file_for_stem() {
  local stem="$1"
  local stats_dir="fyp/dash_results/mawi/flow_stats"
  if [[ ! -d "$stats_dir" ]]; then
    return
  fi
...
split_dir="fyp/dash_results/mawi/cache_sim/source_seen/results_mawi_source_seen_top8_splits"
if [[ -d "$split_dir" ]]; then
...
total_csv="fyp/dash_results/mawi/cache_sim/source_seen/results_mawi_source_seen_top8.csv"
if [[ -f "$total_csv" ]]; then
