...
print_usage() {
  cat <<'EOF'
Usage:
  bash fyp/dash_scripts/run_plot_cache_batch.sh [OUT_BASE] [options]
...
count=0
for csv in fyp/dash_results/synthetic/cache_sim/route_changes/results_synthetic_*.csv; do
  [[ -f "$csv" ]] || continue
  ...
  python3 fyp/dash_scripts/plot_cache.py "$csv" --out "$OUT_SYN_ROUTE" --prefix "$stem" "${EXTRA_ARGS[@]}"
  count=$((count + 1))
done

for csv in fyp/dash_results/synthetic/cache_sim/route_change/results_synthetic_*.csv; do
  [[ -f "$csv" ]] || continue
  stem="$(basename "$csv" .csv)"
  dataset_name="${stem#results_synthetic_}"
  if [[ "$LOW_TEMP_LOCALITY" == "1" && "$dataset_name" != *"_${LOW_TEMP_SUFFIX}" ]]; then
...
for csv in fyp/dash_results/synthetic/cache_sim/source_seen/results_synthetic_source_seen_*.csv; do
  [[ -f "$csv" ]] || continue
  stem="$(basename "$csv" .csv)"
  ...
  echo "== plotting synthetic/$stem =="
  python3 fyp/dash_scripts/plot_cache.py "$csv" --out "$OUT_SYN_SOURCE" --prefix "$stem" "${EXTRA_ARGS[@]}"
  count=$((count + 1))
done
