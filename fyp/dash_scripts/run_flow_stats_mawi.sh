...
print_usage() {
  cat <<'EOF'
Usage:
  bash fyp/dash_scripts/run_flow_stats_mawi.sh [options]

Options:
  -s, --split-dir DIR        Input split directory (default: fyp/dash_dataset/mawi/mawi_sinks_top8_prefix16)
  -o, --out-dir DIR          Output directory (default: fyp/dash_results/mawi/flow_stats)
...
OUT_DIR="${OUT_DIR:-fyp/dash_results/mawi/flow_stats}"
TOP_N="${TOP_N:-20}"
...
for sink_file in "$SPLIT_DIR"/*.txt; do
  ...
  python3 fyp/dash_scripts/flow_stats.py "$sink_file" --top "$TOP_N" --concurrency --csv "$report_csv" > "$report_txt"
  logs+=("$sink_file")
done

if [[ ${#logs[@]} -gt 0 ]]; then
  python3 fyp/dash_scripts/flow_stats.py "${logs[@]}" --top "$TOP_N" --concurrency --csv "$OUT_DIR/mawi_top8_all_flow_stats.csv" > "$OUT_DIR/mawi_top8_all_flow_stats.txt"
fi
