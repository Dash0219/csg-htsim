...
is_supported_dataset_name() {
  ...
  if [[ "$name" =~ ^(incast|a2a)_heavytail_sigma_[0-9]+(p[0-9]+)?$ ]]; then
    return 0
  fi
...
OUT_DIR="fyp/dash_results/synthetic/${PROTOCOL}/cache_sim/route_changes"
...
for p in "${patterns[@]}"; do
  ...
  if [[ "$FAST" == "1" ]]; then
    python3 fyp/dash_scripts/cache_sim.py "$log" --sweep --fast --quiet-table --csv "$csv" --capacity-csv "$capacity_csv"
  else
    python3 fyp/dash_scripts/cache_sim.py "$log" --sweep --quiet-table --csv "$csv" --capacity-csv "$capacity_csv"
  fi

  plot_cmd=(python3 fyp/dash_scripts/plot_cache_capacity.py "$capacity_csv" --out-dir "$dataset_plots_dir" --prefix "synthetic_${p}_capacity" --min-capacity "$CAPACITY_PLOT_MIN_CAPACITY")
