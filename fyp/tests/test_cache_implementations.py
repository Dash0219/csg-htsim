...
from fyp.scripts.cache_sim_congestion_int import (
    AdaptiveAdmissionQS,
    AdmissionFilterQS,
    CacheINTFreshnessQS,
    CongestResult,
...
from fyp.scripts.cache_sim_source_seen import (
    load_records,
    merge_results,
    sanitize_name,
    simulate_source,
...
def test_resolve_inputs_file(tmp_path: Path) -> None:
    from fyp.scripts.cache_sim_source_seen import resolve_inputs
    p = tmp_path / "test.txt"
    p.write_text("col\n1\n")
...
def test_sweep_splitwise(tmp_path: Path) -> None:
    from fyp.scripts.cache_sim_source_seen import sweep_splitwise, load_split_records
...
def test_run_single_splitwise(tmp_path: Path) -> None:
    from fyp.scripts.cache_sim_source_seen import run_single_splitwise
    import argparse as _argparse
    p = tmp_path / "split.txt"
    p.write_text("src_ip\tdst_ip\ttimestamp\n1.1.1.1\t2.2.2.2\t1.0\n3.3.3.3\t4.4.4.4\t2.0\n")
    args = _argparse.Namespace(
...
def test_parse_int_log(tmp_path: Path) -> None:
    from fyp.scripts.cache_sim import parse
...
def test_build_hop_records_switch_level(tmp_path: Path) -> None:
    from fyp.scripts.cache_sim_congestion_int import build_hop_records
    import argparse
...
def test_resolve_inputs_qs_file(tmp_path: Path) -> None:
    from fyp.scripts.cache_sim_congestion_int import resolve_inputs
    p = tmp_path / "log.txt"
