...
print_usage() {
  cat <<'EOF'
Usage:
  bash fyp/scripts/run_plot_cache_synthetic.sh [OUT_DIR] [options]
...
infer_congestion_limits() {
  local dataset_name="$1"
  local log_file="fyp/dataset/synthetic/${PROTOCOL}/log_${dataset_name}.txt"
  if [[ ! -f "$log_file" ]]; then
    return
  fi
...
for PROTOCOL in $PROTOCOLS; do
  ...
  OUT_DIR="${OUT_DIR_BASE:-fyp/results/synthetic/${PROTOCOL}/cache_sim}"
  FLOW_STATS_DIR="fyp/results/synthetic/${PROTOCOL}/flow_stats"
  mkdir -p "$OUT_DIR"

  if [[ "$MODE" == "route_changes" || "$MODE" == "all" ]]; then
    plot_mode "route_changes" \
      "fyp/results/synthetic/${PROTOCOL}/cache_sim/route_changes" \
      "fyp/results/synthetic/cache_sim/route_change/results_synthetic_*.csv" \
      "results_synthetic_"
  fi

  if [[ "$MODE" == "source_seen" || "$MODE" == "all" ]]; then
    plot_mode "source_seen" \
      "fyp/results/synthetic/${PROTOCOL}/cache_sim/source_seen" \
      "" \
  ...
  if [[ "$MODE" == "congestion" || "$MODE" == "all" ]]; then
    plot_mode "congestion" \
      "fyp/results/synthetic/${PROTOCOL}/cache_sim/congestion" \
      "" \
