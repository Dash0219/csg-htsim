...
print_usage() {
  cat <<'EOF'
Usage:
  bash fyp/scripts/run_plot_cache_imc.sh [OUT_BASE] [options]
...
stats_file_for_stem() {
  local ds="$1"
  local stem="$2"
  local stats_dir="fyp/results/imc/$ds/flow_stats"
  if [[ ! -d "$stats_dir" ]]; then
    return
  fi
...
for ds in uni1 uni2; do
  ...
  split_dir="fyp/results/imc/$ds/cache_sim/source_seen/results_${ds}_source_seen_top8_splits"
  if [[ -d "$split_dir" ]]; then
  ...
  total_csv="fyp/results/imc/$ds/cache_sim/source_seen/results_${ds}_source_seen_top8.csv"
  if [[ -f "$total_csv" ]]; then
...
echo "Done. Plotted $count IMC split CSV file(s) to $OUT_BASE/{uni1,uni2}/cache_sim/source_seen/plots"
