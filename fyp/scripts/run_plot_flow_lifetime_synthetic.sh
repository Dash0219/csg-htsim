...
print_usage() {
  cat <<'EOF'
Usage:
  bash fyp/scripts/run_plot_flow_lifetime_synthetic.sh [options]
...
Options:
  ...
  -o, --out-dir DIR         Output directory (default: fyp/results/synthetic/<protocol>/flow_lifetime/plots)
...
for PROTOCOL in $PROTOCOLS; do
  ...
  OUT_DIR="${OUT_DIR_BASE:-fyp/results/synthetic/${PROTOCOL}/flow_lifetime/plots}"
  LOG_ROOT="fyp/dataset/synthetic/${PROTOCOL}"
  mkdir -p "$OUT_DIR"
  ...
  cmd=(python3 fyp/scripts/plot_flow_lifetimes.py --logs-dir "$LOG_ROOT" --out-dir "$OUT_DIR" --prefix "$PREFIX" --max-records "$MAX_RECORDS" --bins "$BINS")
  if [[ -n "$DATASET" ]]; then
    cmd+=(--dataset "$DATASET")
  fi
  ...
  echo "Done [$PROTOCOL]. Synthetic flow lifetime plots are under: $OUT_DIR"
done
