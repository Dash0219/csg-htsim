...
print_usage() {
  cat <<'EOF'
Usage:
  bash fyp/scripts/run_plot_cache_mawi.sh [OUT_BASE] [options]
...
OUT_BASE="${OUT_BASE:-fyp/results/mawi}"
out_dir="$OUT_BASE/cache_sim/source_seen/plots"
mkdir -p "$out_dir"
...
stats_file_for_stem() {
  local stem="$1"
  local stats_dir="fyp/results/mawi/flow_stats"
  if [[ ! -d "$stats_dir" ]]; then
    return
  fi
  ...
  if [[ "$stem" == "results_mawi_source_seen_top8" ]]; then
    local total_stats="$stats_dir/mawi_top8_all_flow_stats.txt"
...
infer_max_concurrency_for_stem() {
  local stem="$1"
  local stats_file
  stats_file="$(stats_file_for_stem "$stem")"
...
run_plot() {
  ...
  python3 fyp/scripts/plot_cache.py "$csv" "${args[@]}"
}

count=0
mawi_max_concurrency="$MAX_CONCURRENCY"
if [[ "$USE_MAX_CONCURRENCY" == "0" ]]; then
  mawi_max_concurrency="0"
fi

mawi_unique_flows="$UNIQUE_FLOWS"
if [[ "$USE_UNIQUE_FLOWS" == "0" ]]; then
  mawi_unique_flows="0"
fi
split_dir="fyp/results/mawi/cache_sim/source_seen/results_mawi_source_seen_top8_splits"
if [[ -d "$split_dir" ]]; then
  for csv in "$split_dir"/*.csv; do
    [[ -f "$csv" ]] || continue
    stem="$(basename "$csv" .csv)"
    csv_max_concurrency="$mawi_max_concurrency"
    if [[ "$USE_MAX_CONCURRENCY" != "0" && ( -z "$csv_max_concurrency" || "$csv_max_concurrency" == "0" ) ]]; then
      inferred_split=$(infer_max_concurrency_for_stem "$stem")
      if [[ -n "$inferred_split" ]]; then
        csv_max_concurrency="$inferred_split"
      fi
    fi
    csv_unique_flows="$mawi_unique_flows"
    if [[ "$USE_UNIQUE_FLOWS" != "0" && ( -z "$csv_unique_flows" || "$csv_unique_flows" == "0" ) ]]; then
      inferred_unique_split=$(infer_unique_flows_for_stem "$stem")
      if [[ -n "$inferred_unique_split" ]]; then
        csv_unique_flows="$inferred_unique_split"
      fi
    fi
    echo "== plotting mawi/$stem =="
...
total_csv="fyp/results/mawi/cache_sim/source_seen/results_mawi_source_seen_top8.csv"
if [[ -f "$total_csv" ]]; then
  stem="$(basename "$total_csv" .csv)"
  csv_max_concurrency="$mawi_max_concurrency"
  if [[ "$USE_MAX_CONCURRENCY" != "0" && ( -z "$csv_max_concurrency" || "$csv_max_concurrency" == "0" ) ]]; then
    inferred_total=$(infer_max_concurrency_for_stem "$stem")
  ...
  csv_unique_flows="$mawi_unique_flows"
  if [[ "$USE_UNIQUE_FLOWS" != "0" && ( -z "$csv_unique_flows" || "$csv_unique_flows" == "0" ) ]]; then
    inferred_unique_total=$(infer_unique_flows_for_stem "$stem")
...
echo "Done. Plotted $count MAWI CSV file(s) to $out_dir"
