...
# Reads fyp/experiments/experiments.txt and runs each entry for 5 seeds:
...
#
# Usage: bash fyp/scripts/run_experiments.sh [--parallel [--jobs N]] [--entry N]

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
cd "$REPO_ROOT"

EXPERIMENTS_FILE="fyp/experiments/experiments.txt"
SCRIPTS="fyp/scripts"
...
if [[ ! -f "$EXPERIMENTS_FILE" ]]; then
  ...
  echo "       Run setup_workspace.sh first, then populate experiments.txt" >&2
  exit 1
fi
...
if [[ -n "$PARALLEL" ]]; then
  ...
  aggregate_entry() {
    ...
    shopt -s nullglob
    local run1_csvs=(fyp/experiments/run1/results/cache_sim/${context}/${protocol}/${csv_glob})
    shopt -u nullglob
    ...
    for run1_csv in "${run1_csvs[@]}"; do
      ...
      for i in 1 2 3 4 5; do
        local candidate="fyp/experiments/run${i}/results/cache_sim/${context}/${protocol}/${csv_basename}"
        if [[ -f "$candidate" ]]; then
  ...
  while true; do
    ...
    for i in "${pending_entries[@]+"${pending_entries[@]}"}"; do
      ...
      for run_idx in $remaining; do
        ...
        (
          run_dir="fyp/experiments/run${run_idx}"
...
for entry in "${ENTRIES[@]}"; do
  ...
  run_one_seed() {
    ...
    local run_dir="fyp/experiments/run${run_idx}"
  ...
  shopt -s nullglob
  run1_csvs=(fyp/experiments/run1/results/cache_sim/${context}/${protocol}/${csv_glob})
  shopt -u nullglob
  ...
  for run1_csv in "${run1_csvs[@]}"; do
    ...
    all_found=1
    for i in 1 2 3 4 5; do
      candidate="fyp/experiments/run${i}/results/cache_sim/${context}/${protocol}/${csv_basename}"
      if [[ -f "$candidate" ]]; then
        csv_inputs+=("$candidate")
      else
        echo "  WARN: missing run${i} CSV: $candidate"
