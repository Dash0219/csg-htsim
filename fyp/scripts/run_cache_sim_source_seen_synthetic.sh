...
print_usage() {
  cat <<'EOF'
Usage:
  bash fyp/scripts/run_cache_sim_source_seen_synthetic.sh [options]
...
for PROTOCOL in $PROTOCOLS; do
  ...
  OUT_DIR="${OUT_DIR:-fyp/results/synthetic/${PROTOCOL}/cache_sim/source_seen}"
  ...
  for p in "${patterns[@]}"; do
    ...
    echo "== source-seen INT sweep [$PROTOCOL]: $p =="

    sim_cmd=(python3 fyp/scripts/cache_sim_source_seen_int.py "$log"
      --route-key seen --sweep --quiet-table --csv "$csv")
    [[ "$FAST" == "1" ]] && sim_cmd+=(--fast)
    ...
    if [[ "$CAPACITY_PLOTS" == "1" ]]; then
      ...
      plot_cmd=(python3 fyp/scripts/plot_cache_capacity.py "$capacity_csv"
        ...
        --prefix "synthetic_source_seen_${p}_capacity"
  ...
  echo "Done [$PROTOCOL]. Source-seen CSV outputs are under: $OUT_DIR"
done
