...
print_usage() {
  cat <<'EOF'
Usage:
  bash fyp/scripts/run_cache_sim_congestion_synthetic.sh [options]
...
for PROTOCOL in $PROTOCOLS; do
  ...
  OUT_DIR="${OUT_DIR:-fyp/results/synthetic/${PROTOCOL}/cache_sim/congestion}"
  ...
  for p in "${patterns[@]}"; do
    ...
    sim_cmd=(python3 fyp/scripts/cache_sim_congestion_int.py "$log"
      --sweep
      --quiet-table
