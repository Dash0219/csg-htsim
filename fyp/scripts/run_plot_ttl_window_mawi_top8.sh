...
print_usage() {
  cat <<'EOF'
Usage:
  bash fyp/scripts/run_plot_ttl_window_mawi_top8.sh [options]

Options:
  -s, --split-dir DIR        Input split directory (default: fyp/dataset/mawi/mawi_sinks_top8_prefix16)
  -o, --out-dir DIR          Output directory (default: fyp/results/mawi/ttl_window/plots)
...
OUT_DIR="${OUT_DIR:-fyp/results/mawi/ttl_window/plots}"
...
if [[ "$PER_SINK" == "1" ]]; then
  for sink_file in "${files[@]}"; do
    python3 fyp/scripts/plot_ttl_windows.py "$sink_file" \
      --out-dir "$OUT_DIR" \
      --prefix "$PREFIX" \
  ...
  python3 fyp/scripts/plot_ttl_windows.py "${files[@]}" \
    --merge-inputs \
