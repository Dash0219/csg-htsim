...
print_usage() {
  cat <<'EOF'
Usage:
  bash fyp/scripts/run_plot_flow_concurrency_synthetic.sh [options]
...
Options:
  ...
  -o, --out-dir DIR         Output directory (default: fyp/results/synthetic/<protocol>/flow_concurrency/plots)
...
for PROTOCOL in $PROTOCOLS; do
  ...
  OUT_DIR="${OUT_DIR_BASE:-fyp/results/synthetic/${PROTOCOL}/flow_concurrency/plots}"
  ...
  cmd=(python3 fyp/scripts/plot_flow_concurrency.py --logs-dir "$LOG_ROOT" --out-dir "$OUT_DIR" --prefix "$PREFIX" --max-records "$MAX_RECORDS" --max-plot-intervals "$MAX_PLOT_INTERVALS")
  if [[ -n "$DATASET" ]]; then
    cmd+=(--dataset "$DATASET")
  fi
