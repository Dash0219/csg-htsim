...
print_usage() {
  cat <<'EOF'
Usage:
  bash fyp/scripts/run_plot_ttl_window_synthetic.sh [options]
...
Options:
  ...
  -o, --out-dir DIR         Output directory (default: fyp/results/synthetic/<protocol>/ttl_window/plots)
...
for PROTOCOL in $PROTOCOLS; do
  ...
  OUT_DIR="${OUT_DIR_BASE:-fyp/results/synthetic/${PROTOCOL}/ttl_window/plots}"
  ...
  cmd=(python3 fyp/scripts/plot_ttl_windows.py --logs-dir "$LOG_ROOT" --out-dir "$OUT_DIR" --prefix "$PREFIX" --max-records "$MAX_RECORDS" --bins "$BINS" --key-level "$KEY_LEVEL" --event-mode "$EVENT_MODE" --qs-threshold "$QS_THRESHOLD")
  if [[ -n "$DATASET" ]]; then
    cmd+=(--dataset "$DATASET")
  fi
